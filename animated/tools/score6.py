"""The score for the complete film (film.html?cut=complete), draft 3: the prologue with its prophecy, chapters I-VIII and the epilogue.

    python3 score6.py cues-complete.json out.wav
...
silences = []
# PROLOGUE: silence, a match, a low string breath and the theme on a music box; the book opens on a prophecy; the crown falls
strike, title, op = m1("strike"), m1("title"), m1("open")
tip, land, pro = m1("tip"), m1("land"), M("prophecy")[0]
...
pad(op, land - op, ["Bb2", "F3", "D4", "F4"], 38)
for n in ["D3", "A3"]:                                        # the prophecy is read over a low hum of voices
    note(CHOIR, pro["t"], land - pro["t"], p(n), 38)
swell(CHOIR, pro["t"], pro["t"] + 3, 30, 96)
for k, n in enumerate(["A4", "Ab4", "G4"]):                 # the crown rocks: three unsteady notes
    note(CEL, tip + k * 0.4, 0.8, p(n), 40)
silences.append({"t": land + 0.35, "d": 1.3})
pad(land + 1.6, m1("begins") - land - 1.6, ["G2", "D3", "Bb3"], 36)
begins = m1("begins")
...
# V · THE GAP: each portrait arrives on a hopeful phrase; the gap stops one note short; the blame is a blow; then a quiet cello
a = t0("portrait")
pr, sw, gap, bl, mb = m1("portrait"), m1("swap"), m1("gap"), m1("blame"), m1("maybe")
pad(a, pr - a, ["D3", "F3", "A3"], 34)
for at in (pr, sw):
    arpeggio(at, ["D4", "F4", "A4", "D5", "E5"], ch=HARP, gap=0.2, vel=40)
pads(pr, gap - 3.6, [["Bb2", "D3", "F3"], ["F2", "A2", "C3", "F3"], ["C3", "E3", "G3"]], 36)
...
pad(gap, bl - gap, ["A2", "E3", "G3", "C#4"], 40)
note(TIMP, bl, 1.6, p("D2"), 78); note(PIANO, bl, 3, p("D1"), 74); note(PIANO, bl, 3, p("Eb2"), 60)
pad(bl, mb - bl, ["D2", "Ab2", "D3", "F3"], 42)
t = mb
for n, dur in [("D3", 2.4), ("C3", 2.0), ("Bb2", 2.4), ("A2", 3.2)]:
    note(CELLO, t, dur, p(n), 50); t += dur
pad(mb, t1("portrait") - mb + 0.5, ["Bb1", "F2", "D3"], 36)
...
note(BASS, kg, t1("mirror") - kg, p("D2"), 50)
# VI · THE PLEDGE: the court kneels at night; the theme, very low; then silence on "no cause."
kn, sil = m1("kneel"), M("silence")[0]
pad(t0("pledge"), sil["t"] - t0("pledge"), ["D2", "A2", "F3"], 36)
for n in ["D3", "A3", "D4"]:
    note(CHOIR, kn, sil["t"] - kn, p(n), 40)
swell(CHOIR, kn, kn + 3, 40, 100)
motif(kn + 1.0, ch=HORN, vel=46, step=1.0)
chords.append((t0("pledge"), [p("D3"), p("F3"), p("A3")]))
silences.append({"t": sil["t"], "d": sil["d"]})
# the ruins at night: out of the silence, slowly; the theme on the piano under "the saddest empires"
r0 = sil["t"] + sil["d"]
pads(r0, t1("ruins"), [["Bb1", "F2", "D3"], ["G1", "D2", "Bb2", "D3"], ["F1", "C2", "A2"], ["D2", "A2", "F3"]], 38)
sad = m1("saddest")
for n in ["D3", "F3", "A3"]:
    note(CHOIR, sad + 1.0, 7, p(n), 38)
motif(sad + 1.4, ch=PIANO, vel=44, step=0.9)
# VII · THE TEST: a figure that multiplies with the hierarchy; a blow at every wall; the walls turn to gold; a door of light
lv, wl, gd, cur, door = M("levels")[0], M("walls")[0], m1("good"), m1("curriculum"), M("door")[0]
a = t0("test")
pad(a, lv["t"] - a, ["D3", "A3", "F4"], 34)
fig, step, t, i = ["D4", "A4", "F4", "A4", "E4", "A4"], 0.3, lv["t"], 0
while t < wl["t"] + 5.2:
    u = (t - lv["t"]) / (wl["t"] + 5.2 - lv["t"])
    note(PIANO, t, step * 0.9, p(fig[i % 6]), 30 + 22 * u)
    if u > 0.4:
        note(PIANO, t, step * 0.9, p(fig[i % 6]) - 12, 22 + 20 * u)
    t += step * (1 - 0.35 * u); i += 1
pads(lv["t"], wl["t"], [["D3", "A3", "F4"], ["Bb2", "F3", "D4"]], 38)
for k in range(5):
    tw = wl["t"] + k * 0.9 + 0.6
    note(TIMP, tw, 1.2, p("A1"), 56 + k * 4); note(PIANO, tw, 1.4, p("D2"), 50 + k * 3); note(PIANO, tw, 1.4, p("Eb2"), 40)
pad(wl["t"], gd - wl["t"], ["D2", "Ab2", "D3", "F3"], 40)
pad(gd, cur - gd, ["F2", "C3", "A3", "F4"], 40)                           # "Good.": the walls turn out to be the way
arpeggio(gd + 0.4, ["F4", "A4", "C5", "F5"], ch=HARP, gap=0.22, vel=38)
pad(cur, door["t"] - cur + 0.5, ["A1", "A2", "E3", "A3", "C#4", "E4"], 48)   # "the curriculum": the dominant, held, with voices
for n in ["E3", "A3", "C#4"]:
    note(CHOIR, cur, t1("test") - cur, p(n), 46)
motif(cur + 0.3, ch=HORN, vel=62, step=0.9)
note(TIMP, cur, 2, p("A1"), 60)
arpeggio(door["t"], ["A3", "C#4", "E4", "A4", "C#5", "E5", "A5"], ch=HARP, gap=0.26, vel=44)
swell(STR, door["t"], t1("test"), 80, 124)
# VIII · THE CROWN: first light; the walk up the aisle; everything hushes as the hall falls away; D major on "Pick it up."
a, wk, fc, pick = t0("crown"), M("walk")[0], M("focus")[0], m1("pick")
pad(a, wk["t"] - a, ["F2", "C3", "A3", "F4"], 40)
arpeggio(a + 1.0, ["D4", "F4", "A4", "D5", "F5", "A5"], ch=HARP, gap=0.14, vel=40)
seq = [["Bb2", "F3", "D4"], ["G2", "D3", "Bb3"], ["C3", "G3", "E4"]]
st = (fc["t"] - wk["t"]) / len(seq)
for k, names in enumerate(seq):
    pad(wk["t"] + k * st, st, names, 40 + k * 4)
    for b_ in np.arange(0, st, 0.62):
        note(CELLO, wk["t"] + k * st + b_, 0.55, p(names[0]) - 12, 42 + k * 6)
swell(STR, wk["t"], fc["t"], 70, 112)
pad(fc["t"], pick - fc["t"], ["A2", "E3", "A3", "C#4"], 36)                # the hall falls away: only a held chord
swell(STR, fc["t"], fc["t"] + 2.5, 112, 70)
pad(pick, DUR - pick, ["D2", "A2", "D3", "F#3", "A3", "D4"], 44)
for n in ["D3", "F#3", "A3", "D4"]:
    note(CHOIR, pick + 0.2, 9, p(n), 42)
motif(pick + 0.4, major=True, vel=60, step=0.8)
for k, n in enumerate(["A5", "D6", "F#6", "A6", "D7"]):
    note(CEL, pick + 0.2 + k * 0.16, 2.5, p(n), 38 - k * 3)
# EPILOGUE: back into the book; the rest of it is blank; it closes; the candle goes out
zo = M("open")[-1]
arpeggio(zo["t"] + 0.3, ["A5", "F#5", "D5", "A4", "F#4", "D4"], gap=0.16, vel=34)
bk = m1("blank")
for k, n in enumerate(["A5", "F#5", "E5"]):
    note(CEL, bk + 0.3 + k * 0.7, 2, p(n), 34)
close = M("close")[0]["t"]
